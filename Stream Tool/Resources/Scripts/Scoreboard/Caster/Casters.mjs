...
class Casters {
    ...
    async update(data) {
        ...
        if (incCasterLength != homeCasterLength) {
            ...
            if (!current.startup) { // skip if loading up
                await fadeOut(castersDiv.firstElementChild, fadeOutTimeSc);
            }
            ...
            fadeIn(castersDiv.firstElementChild, fadeInTimeSc, .3);
    ...
    #checkEmpty() {
        ...
        if (notEmpty) {
            ...
            fadeIn(castersDiv, fadeInTimeSc);
        } else {
            castersDiv.style.height = "0px";
            castersDiv.style.borderWidth = "0px";
            fadeOut(castersDiv, fadeOutTimeSc);
        }
